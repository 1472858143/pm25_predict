...
PM2.5 多模型预测系统，基于北京小时级空气质量数据。默认实验：过去 720 小时 → 未来 72 小时直接多输出预测。支持 LSTM、XGBoost、RandomForest、ARIMA、SARIMA 五种模型。
...
# 跨模型比较
python -m pm25_forecast.scripts.compare_models --models lstm xgboost random_forest arima sarima
...
**路径工具**（`utils/paths.py`）：集中管理路径构造。`SUPPORTED_MODEL_NAMES = ("lstm", "xgboost", "random_forest", "arima", "sarima")`。所有脚本通过 `validate_model_name()` 校验模型名。
